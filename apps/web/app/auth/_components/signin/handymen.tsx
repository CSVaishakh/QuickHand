...
type HandymenSigninForm = {
	email: string;
	password: string;
};

export default function HandymenSigninForm () {
	const router = useRouter();

	const [form, setForm] = useState<HandymenSigninForm>({
		email: "",
		password: "",
	});
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			const response = await fetch(`${baseUrl}/handyman/sign-in`, {
				method: "POST",
